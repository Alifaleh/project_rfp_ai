...
class RfpProject(models.Model):
    ...
    def _notify_stage_progress(self, stage):
        ...
        stage_messages = {
            STAGE_CONTENT_GENERATED: {
                'chatter': '<div class="alert alert-info"><strong>Content Generated!</strong> All section content has been generated by AI. Image generation will begin automatically.</div>',
                'title': 'RFP Content Ready',
                'message': f'Content generation for "{self.name}" is complete. Image generation starting...',
            },
            STAGE_IMAGES_GENERATED: {
                'chatter': '<div class="alert alert-success"><strong>Images Generated!</strong> All diagram images have been generated. Your RFP project is ready for review.</div>',
                'title': 'RFP Images Ready',
                'message': f'All images for "{self.name}" are generated. Your project is ready for review!',
                'template': 'project_rfp_ai.mail_template_rfp_generation_complete',
            },
        }

        info = stage_messages.get(stage)
        if not info:
            return

        # Chatter notification
        self.message_post(
            body=Markup(info['chatter']),
            message_type='notification',
            subtype_xmlid='mail.mt_note'
        )

        # Email (only for images_generated - the final generation milestone)
        template_ref = info.get('template')
        if template_ref:
        ...
        for project in self:
            ...
            if status_data['status'] == 'completed':
                if project.current_stage == STAGE_GENERATING_CONTENT:
                     ...
                     project._notify_stage_progress(STAGE_CONTENT_GENERATED)
                     # Auto-Trigger Images
                     ...
                     project._notify_stage_progress(STAGE_IMAGES_GENERATED)
